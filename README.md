...
> which is current `main`.**  It supersedes `cc3501e-v0.5.0.bin`, which went
> stale the moment #84 landed the SPI1 passthrough; the older blobs are kept
...
>
> **What 0.6.0 adds:** the E1M connector's SPI1 relayed through the bridge
> (#83) -- opcodes `0x55` / `0x56` / `0x57` -- which bumps the wire protocol to
> **6**.  A host built against v5 is refused by `GET_VERSION`; pair this blob
...
#    THIS artifact's stamp (0.6.0.0) -- see the warning below.  An existing
#    flash-set built at another version will NOT do; regenerate it:
#      VERSION=0.6.0.0 ti/regen_flashset.sh
#
...
interchangeable -- app SemVer (`0.6.0`), wire protocol (`6`), and the GPE
image stamp (`0.6.0.0`).  `prebuilt/CHANGELOG.md` has the table.

> **STOP -- check the unit's flash history before using this artifact's `0.6.0.0`
...
> far above `0.6.0.0` -- the bench part here is at `0.149.70.0`.  For such a unit,
...
protocol and a host built from this tree refuses all of them: `cc3501e-v0.5.0.bin`,
`cc3501e-v0.4.1.bin` and `cc3501e-v0.4.0.bin` answer **5**, `cc3501e-v0.3.0.bin`
answers **4**.  They are kept for traceability only -- and among them 0.4.1
...
| `prebuilt/` populated | ✅ `cc3501e-v0.6.0.bin` signed (full bridge: META + Wi-Fi + BLE + OTA + the E1M SPI1 passthrough, **proto v6**). Matches `main` as of the v0.6.0 release commit, and `prebuilt freshness` keeps that true. Carries 0.5.0's socket-EOF repair (#32) and the BLE `scan-stop` re-init removal that took the #5 wedge from ~9% to ~1%. **Sockets still do not connect from the host** (alp-sdk#1746) — which is also why #32 is not yet exercised on silicon; see `prebuilt/CHANGELOG.md`. |

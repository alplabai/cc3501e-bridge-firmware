...
- WIFI_SCAN: `scanCommon_t sc={.Band=BAND_SEL_BOTH}; Wlan_Scan(WLAN_ROLE_STA,&sc,N);` → wait `WLAN_EVENT_SCAN_RESULT` (ONE event, whole list in `Data.ScanResult.NetworkListResult[]`, `WlanNetworkEntry_t{Ssid[32],Bssid[6],SsidLen,Rssi,SecurityInfo,Channel}`). Pack compact (`ALP_CC3501E_MAX_PAYLOAD` is 4096 as of protocol v5 ⇒ ~140 entries/frame at the 11-byte scan record + SSID; page if needed).

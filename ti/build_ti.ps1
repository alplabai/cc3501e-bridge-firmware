...
# VERIFY, do not assume -- and verify the COUNT, not just presence.  A bare -match
# only proves at least ONE SPI_Config entry patched, but the -WifiHostDriver board
# file declares TWO (spi0 for the bridge, spi1 for this family's own controller):
# a -replace that matched only one (say SysConfig ever emits spi1's entry with
# different formatting) would still pass a presence check while leaving spi1's
# polled payload phases in the DMA branch -- unpatched and silent.  Count matches
# against the number of SPI_Config entries this board file actually declares: 2
# under -WifiHostDriver, 1 on the default (spi0-only) board.  Same post-condition
# discipline as the .TI.noinit patch.
$expectedSpiInstances = if ($WifiHostDriver) { 2 } else { 1 }
$patchedCount = ([regex]::Matches((Get-Content $cfg -Raw), [regex]::Escape(".minDmaTransferSize = 1024,"))).Count
if ($patchedCount -ne $expectedSpiInstances) {
    throw "build_ti.ps1: minDmaTransferSize patched $patchedCount of $expectedSpiInstances SPI_Config entries in $cfg (SysConfig output drifted). Polled payload phases would fall into the DMA branch and wedge psa_fwu."
}
...
$sources = @(
    ...
    "$fw\src\protocol_meta.c", "$fw\src\protocol_gpio.c", "$fw\src\protocol_spi.c",
    ...
    "$fw\src\wifi_retry.c",
    "$fw\src\wifi_connect_fail_skip.c",
...
# SPI1 host passthrough HAL body (protocol v6): CONFIG_SPI_1 only exists in the
# Wi-Fi board file (cc3501e_aen_wifi.syscfg's "SPI1 controller (master)" block).
# The real HAL (cc3501e_hw_ti_spi_master.c) #errors without CONFIG_SPI_1, so it
# must never be linked into the default (non -WifiHostDriver) image -- that
# board file (cc3501e_aen.syscfg) keeps SPI_count == 1 on purpose, to stay
# byte-identical to the silicon-validated bench build.  worker.c still calls
# cc3501e_hw_spi1_configure/transfer/release unconditionally, so the default
# image links the NOTIMPL stand-in instead (RESP_ERR_NOT_READY on every call).
if ($WifiHostDriver) {
    $sources += "$fw\hal\ti\cc3501e_hw_ti_spi_master.c"
} else {
    $sources += "$fw\hal\ti\cc3501e_hw_ti_spi_master_notimpl.c"
}
